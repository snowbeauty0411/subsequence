      ...
      ><b-row
        ><b-col cols="12" lg="6" class="g-0"
          ><div class="editService-delivery-left flex flex-col justify-between">
            <div class="editService-delivery-left-form">
              ...
              <div class="step1-form" v-if="selected === '1'">
                <b-button
            ...
            <div class="editService-step2-left-btn">
              <div
                class="text-danger"
...
export default {
  ...
  data() {
    return {
      ...
      time_distance: null,
      listCourse: [],
      messageReserve: Message.DELIVERY,
    };
  },
  ...
  created() {
    ...
    let service_reserve_setting_init = JSON.parse(
      localStorage.getItem("service_reserve_setting-init")
    );
    let service_reserve_setting = JSON.parse(
      localStorage.getItem("service_reserve_setting")
    );

    if (this.isEditDraftService) {
      if (
        service_reserve_setting_init !== null &&
        service_reserve_setting === null
      ) {
        this.service_reserve_setting = service_reserve_setting_init;
      } else if (
        service_reserve_setting_init !== null &&
        service_reserve_setting !== null
      ) {
        localStorage.setItem("service_reserve_setting_init", null);
    ...
    let is_reserves = localStorage.getItem("is_reserves-init");
    ...
    if (this.service_reserve_setting !== null) {
      ...
      if (this.service_reserve_setting.is_enable) {
        this.checked = true;
      } else {
        this.checked = false;
      }
      ...
      this.duration_before =
        this.service_reserve_setting.duration_before === 0
          ? ""
          : this.service_reserve_setting.duration_before;
      this.duration_after1 =
        this.service_reserve_setting.type_duration_after === 0 ||
        this.service_reserve_setting.type_duration_after === 1
          ? this.service_reserve_setting.duration_after
          : null;
      this.duration_after2 =
        this.service_reserve_setting.type_duration_after === 2
          ? this.service_reserve_setting.duration_after
          : null;
      this.selected3 =
        this.service_reserve_setting.type_duration_after === 2 ? 1 : 0;
      // }
    }
  },
  ...
  methods: {
    ...
    changeChecked() {
      ...
      // if (this.checked) {
      //   localStorage.setItem("is_enable", 1);
      // } else {
      //   localStorage.setItem("is_enable", 0);
      // }
    },
    changeMax() {
      if (this.selected3 === "0") {
      ...
      if (Number(this.selected) === 0) {
        ...
        if (this.max === null && Number(this.selected2) === 1) {
          ...
          if (this.isEditDraftService) {
            ...
            this.$router.push(`/seller/service/settingHour?hash_id=${hash_id}`);
          } else {
    ...
    submitModal() {
      if (/^[０-９]/.test(this.max)) {
        this.toast.error(this.messageReserve.enter_halfwidth, this.optionToast);
        return;
      }
      if (/^[０-９]/.test(this.duration_before)) {
        this.toast.error(this.messageReserve.enter_halfwidth, this.optionToast);
        return;
      }
      if (/^[０-９]/.test(this.duration_after1)) {
        this.toast.error(this.messageReserve.enter_halfwidth, this.optionToast);
        return;
      }
      if (/^[０-９]/.test(this.duration_after2)) {
        this.toast.error(this.messageReserve.enter_halfwidth, this.optionToast);
        return;
      }
      if (this.max === null && Number(this.selected2) === 1) {
