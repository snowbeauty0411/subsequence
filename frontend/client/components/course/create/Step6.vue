...
export default {
  ...
  methods: {
    ...
    nextStep() {
      ...
      if (listCourse.length > 0) {
        ...
        if (lastCourse.index === this.index) {
          this.$router.push(
            `/seller/service/service-course/create?step=7&index=${this.index}`
          );
        } else {
          this.$router.push(
            `/seller/service/service-course/create?step=1&index=${
              this.index + 1
            }`
          );
        }
    ...
    backStep() {
      this.$router.push(
        `/seller/service/service-course/create?step=5&index=${this.index}`
      );
    },
  },
};
