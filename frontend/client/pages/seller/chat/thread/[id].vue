    ...
    <div class="threadChatId-content">
      <div class="threadChatId-content-top">
        <div class="threadChatId-content-top-left flex items-center">
          <div class="item-avatar" v-on:click="$router.push({path: `/profile/${partner.partner_id}`})">
            <img
              :src="
          ...
          <div class="item-name" v-on:click="$router.push({path: `/profile/${partner.partner_id}`})">{{ partner.partner_name }}</div>
        </div>
      </div>
      <div
        v-if="preloadList"
          ...
          <div class="main-chat-content">
            ...
            >
              <div class="flex w-1/2 chat-left-item">
                <div class="chat-left-avatar" v-on:click="$router.push({path: `/profile/${partner.partner_id}`})">
                  <img
                    :src="
                      partner.partner_avatar_img_url
                        ? partner.partner_avatar_img_url
...
export default {
  ...
  computed: {
    ...mapState(ChatStore, ["success", "message", "listMessages", "error"]),
  ...
  methods: {
    ...
    sendMessage() {
      ...
      if (this.file === "" && this.message_content === "") {
        this.toast.error(
          this.messageChat.message_content_is_required,
          this.optionToast
        );
      } else {
    ...
    onFileChangeFile(e) {
      ...
      if (e.target.files[0].size / 1024 / 1024 > 10) {
        this.toast.error(this.messageChat.max_size_file, this.optionToast);
      } else {
